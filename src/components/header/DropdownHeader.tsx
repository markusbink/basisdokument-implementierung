...
export const DropdownHeader: React.FC<any> = () => {
  ...
  return (
    <div className="flex flex-row gap-4 p-2 pl-8 pr-8 bg-white items-center">
      ...
      <div>
        <span className="font-extrabold tracking-widest text-xs">SORTIERUNGEN</span>
      ...
      <div>
        ...
        <div
          className={cx(`flex flex-col lg:flex-row items-center h-12 lg:h-8 gap-2 lg:gap-4 text-sm font-medium`, {
            "opacity-30 pointer-events-none": (getCurrentTool.id === Tool.Highlighter || getCurrentTool.id === Tool.Eraser),
          })}
        >
          ...
          <div className="flex flex-row items-center gap-2">
            <input className="small-checkbox accent-darkGrey cursor-pointer" type="checkbox" checked={hideEntriesHighlighter} onChange={() => setHideEntriesHighlighter(!hideEntriesHighlighter)} />
            <span
      ...
      <div className="h-12 w-0.5 bg-lightGrey rounded-full"></div>
      <div>
        <span className="font-extrabold tracking-widest text-xs">ÄNDERUNGEN VON</span>
        <div className="flex flex-row items-center h-8 gap-2">
          <input
